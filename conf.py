...
# Print high-frequency MG-MTTA per-step parameter/gradient diagnostics.
_C.LOG.VERBOSE_DIAG = False

# Print per-batch metric blocks in stdout (metrics are still saved to files).
_C.LOG.BATCH_METRICS_STDOUT = False

# Print prompt-bank preview texts at startup.
_C.LOG.PROMPT_PREVIEW = False

# Print input stats before/after model normalization.
_C.LOG.INPUT_STATS = False
...
# timm: <redacted_url>
# RobustBench: <redacted_url>
# OpenCLIP: <redacted_url>
_C.MODEL.ARCH = 'Standard'

# Type of pre-trained weights
# For torchvision models see: https://pytorch.org/vision/0.14/models.html
# For OpenClip models, use either 'openai' (for the original OpenAI weights) or see <redacted_url>
...
# Check <redacted_url> for corruption details
...
_C.CLIP.STRICT_OPENAI_ARCH = True               # Enforce ViT-B-16-quickgelu for OpenAI/OpenCLIP ViT-B/16 checkpoints
...
# matched subspace parameter selection
_C.MGMTTA.UPDATE_PATTERNS = [
    "prompt_learner",
    "adapter",
    "gate",
    "ln",
...
# entropy + diversity + gate coefficients
_C.MGMTTA.LAMBDA_GATE = 0.1
_C.MGMTTA.LAMBDA_DIV = 0.01

# gate settings
_C.MGMTTA.GATE_HIDDEN_DIM = 8
_C.MGMTTA.GATE_GRANULARITY = "batch"   # batch / sample
_C.MGMTTA.FIXED_ALPHA = -1.0           # < 0 means learn alpha

# reliability / anchor settings
_C.MGMTTA.ANCHOR_CONF_THRESHOLD = 0.7
_C.MGMTTA.ANCHOR_MOMENTUM = 0.9
_C.MGMTTA.RELIABILITY_TAU = 5.0
...
# ------------------------------- Text Shift options ---------------------- #
_C.TEXT_SHIFT = CfgNode()

# master switch
_C.TEXT_SHIFT.ENABLED = False

# choose from: clean, text_only, joint, failure
_C.TEXT_SHIFT.REGIME = "clean"

# choose from: main, failure
_C.TEXT_SHIFT.PROTOCOL = "main"

# main: prompt_bank_degradation, tokenization_stress, position_context_stress
# failure: negation, wrong_attribute, class_inconsistent
_C.TEXT_SHIFT.FAMILY = "prompt_bank_degradation"

# perturbation strength / variant index
_C.TEXT_SHIFT.LEVEL = 1

# whether to keep a clean bank alongside shifted bank
_C.TEXT_SHIFT.KEEP_CLEAN_BANK = True

# ------------------------------- Unified Output options ------------------ #
_C.EXPERIMENT_OUTPUT = CfgNode()

# Root directory for unified experiment artifacts
_C.EXPERIMENT_OUTPUT.ROOT = "./outputs"

# Batch-collapse rule used for collapse_rate aggregation
_C.EXPERIMENT_OUTPUT.COLLAPSE_TOP1_RATIO_THRESHOLD = 0.8
_C.EXPERIMENT_OUTPUT.COLLAPSE_MARGINAL_ENTROPY_THRESHOLD = -1.0

# Failure analysis thresholds
_C.EXPERIMENT_OUTPUT.FAILURE_LOW_ENTROPY_THRESHOLD = 0.5
_C.EXPERIMENT_OUTPUT.FAILURE_HIGH_CONFLICT_THRESHOLD = 0.5
_C.EXPERIMENT_OUTPUT.FAILURE_TOPK = 50

# Run summary settings
_C.EXPERIMENT_OUTPUT.TOP_K_BEST_WORST = 5
...
def assert_and_infer_cfg():
    ...
    assert _C.MODEL.ADAPTATION in [
        "source", "norm", "tent", "sar", "tpt", "vte", "batclip",
        "entropy_only_matched", "entropy_diversity", "mg_mtta",
        "cotta", "rmt", "rotta", "adacontrast", "gtta", "lame",
        "memo", "eata", "rpl", "roid", "santa", "cmf", "deyo", "ours"
    ]
...
def load_cfg_from_args(description="Config options."):
    ...
    log_base = os.path.splitext(os.path.basename(args.cfg_file))[0]
    text_enabled = bool(getattr(cfg.TEXT_SHIFT, "ENABLED", False))
    if text_enabled:
        text_family = str(getattr(cfg.TEXT_SHIFT, "FAMILY", "unknown")).strip().lower().replace(" ", "-")
        text_level = int(getattr(cfg.TEXT_SHIFT, "LEVEL", 1))
    else:
        text_family = "clean"
        text_level = 0
    log_dest = f"{log_base}_{text_family}_lvl{text_level}_{current_time}.txt"
