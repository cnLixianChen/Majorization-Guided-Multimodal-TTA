...
class TextEncoder(nn.Module):
    def __init__(self, clip_model):
        ...
        # whether the transformer expects batch first (N, L, D)
        self.batch_first = getattr(self.transformer, 'batch_first', False)

    def forward(self, prompts, tokenized_prompts):
        x = prompts + self.positional_embedding.type(self.dtype)
        # transformer may expect batch-first (N, L, D) or seq-first (L, N, D)
        if self.batch_first:
            # prompts is (N, L, D) already
            x = self.transformer(x, attn_mask=self.attn_mask)
        else:
            # convert to (L, N, D) for transformer, then convert back
